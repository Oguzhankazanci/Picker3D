...
public class LevelEditor : Editor
{
    ...
    private static int picker1 = 0;
    private static int picker2 = 0;
    private static int picker3 = 0;
    private static List<int> Pickers = new List<int>();
    ...
    private static void DrawSaveLoadButtons(SceneView sceneView)
    {
        ...
        GUI.Label(new Rect(sceneView.position.width - 170, 120, 50, 20), "LEVEL");
        GUI.contentColor = Color.white;
        levelIndex = EditorGUI.IntField(new Rect(sceneView.position.width - 130, 120, 100, 20), levelIndex);
        ...
        GUI.Label(new Rect(sceneView.position.width - 170, 180, 50, 20), "Picker1");
        GUI.contentColor = Color.white;
        picker1 = EditorGUI.IntField(new Rect(sceneView.position.width - 130, 210, 100, 20), picker1);
        GUI.Label(new Rect(sceneView.position.width - 170, 240, 50, 20), "Picker2");
        GUI.contentColor = Color.white;
        picker2 = EditorGUI.IntField(new Rect(sceneView.position.width - 130, 270, 100, 20), picker2);
        GUI.Label(new Rect(sceneView.position.width - 170, 300, 50, 20), "Picker3");
        GUI.contentColor = Color.white;
        picker3 = EditorGUI.IntField(new Rect(sceneView.position.width - 130, 330, 100, 20), picker3);
    ...
    {
        GUI.Label(new Rect(sceneView.position.width - 280, 20, 100, 20), "OBJECTS");
    ...
    {
        GUILayout.BeginArea(new Rect(0, sceneView.position.height - 40, sceneView.position.width, 20), EditorStyles.toolbar);
        {
            string[] buttonLabels = new string[] { "Remove", "Add" };
    ...
    private static void AddObject()
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hit, 1000, generator.GroundLayer))
        {
            if (hit.collider.gameObject)
            {
                if (SelectedObjectId =="Ball")
                {
                    Level.LevelObjectData data = new Level.LevelObjectData(new Vector3(hit.point.x,0.13f, hit.point.z), Quaternion.identity, generator.AvaliableObjects[GetPrefabID(SelectedObjectId)].transform.localScale, SelectedObjectId);
                    generator.CreateNewObject(data);
                }
                else
                {
                    Level.LevelObjectData data = new Level.LevelObjectData(hit.point, Quaternion.identity, generator.AvaliableObjects[GetPrefabID(SelectedObjectId)].transform.localScale, SelectedObjectId);
                    generator.CreateNewObject(data);
                }
    ...
    private static int GetPrefabID(string objectName)
    {
        int id = 0;
        for (int i = 0; i < generator.AvaliableObjects.Length; i++)
        {
            if (objectName == generator.AvaliableObjects[i].name)
            {
                id = i;
            }
        }
    ...
    private static void NewLevel()
    {
        ...
        AssetDatabase.CreateAsset(level, LEVELS_PATH + Level_Name + levelIndex + ".asset");
        generator.GenerateLevel(level);
    }

    private static void LoadLevel()
    {
        if (string.IsNullOrEmpty(Level_Name + levelIndex))
        {
            EditorUtility.DisplayDialog("Level name empty", "Set level name before loading", "Yes", "Yes");
            return;
        }

        level = null;
        TextAsset textAssetMap = Resources.Load<TextAsset>("Levels/Texts/" + Level_Name + levelIndex);
        Level loadedLevel = Resources.Load<Level>("Levels/" + Level_Name + levelIndex);
        ...
        if (loadedLevel.pickerValues!=null)
        {
            if (loadedLevel.pickerValues.Count ==3)
            {
                picker1 = loadedLevel.pickerValues[0];
                picker2 = loadedLevel.pickerValues[1];
                picker3 = loadedLevel.pickerValues[2];
            }
    ...
    private static void CreateTextAsset()
    {
        ...
        string mydocpath = Application.dataPath + "/Resources/Texts/";
        using (StreamWriter writer = new StreamWriter(Path.Combine(mydocpath, Level_Name + levelIndex + ".txt")))
        {
            writer.Write(data);
        }
    ...
    {
        string data = Resources.Load<TextAsset>("Levels/Texts/" + Level_Name + levelIndex).text;
        if (level == null)
        {
            level = CreateInstance<Level>();
            ...
            if (!string.IsNullOrEmpty(data))
                AssetDatabase.CreateAsset(level, "Assets/Resources/Levels/" + Level_Name + levelIndex + ".asset");
        }
    }
    ...
    private static void UpdateLevel()
    {
        ...
        Pickers.Add(picker2);
        Pickers.Add(picker3);
        level.AddPickerList(Pickers);

        //SaveLoadManager.SetPickers(levelIndex,Pickers);
    }
